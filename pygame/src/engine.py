...
class Engine:
    ...
    def open(self, filename):
        try:
            dirname = os.path.dirname(filename) # 指定したファイルのフォルダ名を取得
            if not dirname:
                path = self.dirname + '/' + filename
            else:
                self.dirname = dirname
                path = filename
            with open(path, 'r') as file: # pathで指定したファイルを読み込み(r)、その内容を変数fileに代入する。path=filename=open関数の引数。
                self.lineno = 0
                self.lines = file.readlines() # fileの中身を1行ずつ読み込みリスト化して代入
                self.show_keyboard = False
                self.text = ''
                self.hint = ''
                self.zenkaku = False
    ...
    def run(self, keyboard):
        if not self.lines: # 中身のないファイルが読み込まれた場合
            return False
        ...
        while self.mode in (EngineMode.RUN, EngineMode.TEXT, EngineMode.HINT):
            if len(self.lines) <= self.lineno: # fileから取得したlinesリストの要素数とlineno変数（初期値0）を比較（lineno:linesの参照する要素数を管理する変数）
                self.lineno = 0
            line = self.lines[self.lineno] # linesの要素を参照して代入
            if line and line[0] == ':': # line（要素）がある且つ文字列の最初が':'の場合
                line = line.rstrip() # lineからスペースを削除したものを代入
            self.lineno += 1 # 参照するlinesの要素を+1

            if self.mode in (EngineMode.TEXT, EngineMode.HINT):
                if not line.startswith(':'): # lineの文字列の最初の文字が':'で無い場合
                    text += line # 変数textにlinesの要素（line（文字列）を追加
                    continue # 処理を終了してwhileの先頭に戻る
                text = text.rstrip() # textのスペースを削除して代入
