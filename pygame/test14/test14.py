...
class PyAction:
    def __init__(self):
        ...
        # 画像のロード
        # ロックマン右向き
        Megaman.right_image = load_image("d:/work/pygame/test14/data/megaman.png", -1)
        # ロックマン左向き
        Megaman.left_image = pygame.transform.flip(Megaman.right_image, 1, 0)
        # ブロック
        Block.image = load_image("d:/work/pygame/test14/data/block2.png", -1)
        
        # マップのロード
        self.map = Map("d:/work/pygame/test14/data/test.map")
        
        # メインループ
        clock = pygame.time.Clock() # fpsを設定
        while True:
            clock.tick(60) # 60fps
            self.update()
            self.draw(screen)
...
class Map:
    ...
    def __init__(self, filename):
        # スプライトグループの登録
        self.all = pygame.sprite.RenderUpdates() # 描画用グループall作成
        self.blocks = pygame.sprite.Group() # 衝突判定用グループblocks作成
        Megaman.containers = self.all # ロックマンはallに東郎
        Block.containers = self.all, self.blocks # ブロックはallとblocks
        
        # プレイヤーの作成
        self.Megaman = Megaman((300,200), self.blocks)
    ...
    def draw(self):
        ...
        self.surface.fill((0,0,0)) # surface背景を指定色で塗りつぶし
        self.all.draw(self.surface) # surfaceに描画
