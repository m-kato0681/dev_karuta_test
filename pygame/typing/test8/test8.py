""" pygame """
""" test7 + 日本語の扱い & 入力した文字をファイルに上書き&読み込み """
...
# 読み書き用ファイルのパスを定義
memo = ['memo1.txt', 'memo2.txt'] # タイピング課題の問題が記載されているテキスト
qa_memo = ['question_and_answer.txt'] # 質問を表示したり回答を入力するためのテキスト
...
def file_write(name, add_txt): # ファイル書き込み
    with open(name, 'a', encoding='utf-8') as f2: # 指定したファイルを書き込み用で開いてf2と名づける
        new_txt = f2.write(add_txt) # 引数で指定した文字列をファイルの最終行に書き込み
        return new_txt
...
def main():
    ...
    font  = pg.font.Font(None, 32) # デフォルトの文字フォントと大きさを定義
    jfont = pg.font.Font('ipag.ttf', 32) # 日本語表示用のフォントを定義
    ...
    true_word = file_read(qa_memo[target_file]) # ★日本語表示実験用
    ...
    while not done:
        ...
        for i in range(len(true_word)): # お手本文字配列の要素数分ループ
            if i == target_line: # 該当箇所と同じ場合
                true_txt_surface = jfont.render(true_word[i], True, color_active) # 該当文字列は色を変えてrenderにセット
            else:
                true_txt_surface = jfont.render(true_word[i], True, color_inactive) # お手本文字列をrenderにセット
            screen.blit(true_txt_surface, (100, 100+i*100)) # 指定位置にお手本文字列を描画

        for i in range(len(display_text)): # 表示用文字配列の要素数分ループ
            display_text_surface = jfont.render(display_text[i], True, color_true)
        ...
        if len(text) > 0: # 入力文字列textの文字数が0より大きい場合
            ...
            if text[-1] == true_word[target_line][len(text)-1]: # 入力文字とお手本が等しい場合
                typing_stop = 0
                input_txt_surface = jfont.render(text, True, color_true)
                ...
                if typing_stop == 0:
                    ...
                    miss_typing += 1
                    print('ミス：', miss_typing)
                input_txt_surface = jfont.render(text, True, color_false)
                screen.blit(input_txt_surface, (100, 120+target_line*100)) # 指定位置に入力文字列を描画
