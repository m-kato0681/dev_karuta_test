...
<head>
    ...
    <title>小・中学生のための国際ロボット競技会 UNIVERSAL ROBOTICS CHALLENGE アイデアコンテスト 小学校部門 投票サイト 待機画面</title>
    <!-- CSS -->
...
<body>
    ...
    <main class="text-center">
        ...
        <a href="https://www.urc21.org/test/urc2022_test/">
            <button name="to_topbutton" type="button" class="col-6 mx-1 btn btn-warning">2022 UNIVERSAL ROBOTICS CHALLENGE のHPへ</button>
        </a>
        <div id="other_movie_comment">
            エントリー中の他の作品も見てみよう！
        </div>
        <!-- 一覧表示用の動画ID格納配列を作成 -->
        <?php
        ...
        <div class="container">
            ...
            <div class="row">
                ...
                <?php
                    ...
                    // id数量分コードを読み込み
                    foreach ($mov_id_array as $i) {
                        include "user_container_col_btnless.php";
                        $id_num ++;
                    }
                ?>
            </div>
        ...
        function searchButton_click() {
            ...
            if (searchText.value!="") { // 検索欄が空ではない場合
                ...
                for (var i=0; i<searchText_array.length; i++) {
                    ...
                    let reg_fullw = new RegExp('(' + Object.keys(kata_fullw).join('|') + ')', 'g');
                    ...
                    searchText_fullw = searchText_halfw.replace(/\"/g, "”").replace(/'/g, "’").replace(/`/g, "‘").replace(/\\/g, "￥").replace(/ /g, "　").replace(/~/g, "〜");
                    // コードシフトで変換できないその他の全角を半角に変換
                    searchText_halfw = searchText_halfw.replace(/”/g, "\"").replace(/’/g, "'").replace(/‘/g, "`").replace(/￥/g, "\\").replace(/　/g, " ").replace(/〜/g, "~");
