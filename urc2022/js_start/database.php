<?php
	...
	try {
		foreach ($form_array as $form_elem) {
			$stt = $db->prepare('SELECT num_of_votes FROM voting_result_js WHERE movie_id = :id');
			...
			if($row = $stt->fetch(PDO::FETCH_ASSOC)) { // 該当するDBのデータを1行取得
				// 既に同じIDがあった場合
				$stt = $db->prepare('UPDATE voting_result_js SET num_of_votes = :num WHERE movie_id = :id');
				$stt->bindValue(':num', $row['num_of_votes'] + 1); // 現在の投票数に+1
				$stt->bindValue(':id', $form_elem);
			}
			else { // 同じIDがない場合
				$stt = $db->prepare('INSERT INTO voting_result_js (movie_id, num_of_votes) VALUES (:id, :num)');
